...
import { View,Text,TextInput,ScrollView,TouchableOpacity,AsyncStorage,RefreshControl, Image,Picker, ActivityIndicator, FlatList } from 'react-native';
...
import { getMyPreviousList, myPreviouspostStartLoading, myPulltorefreshPreviousPost } from '../../actions/MyPreviousPostActions';
import { Actions } from 'react-native-router-flux';
const mapStateToProps = ({ TravelersListReducer, MyPreviousPostReducer, LoginReducer, SignUpReducer }) => {

  return {
    travelersListResponse: TravelersListReducer.travelersListResponse,
    loading: MyPreviousPostReducer.isLoading,
    previousListResponse: MyPreviousPostReducer. previousListResponse,
    isRegistered: SignUpReducer.isRegistered,
    loggedIn: LoginReducer.loggedIn,
    pulltorefresh: MyPreviousPostReducer.pulltorefresh
  };
};
const mapDispatchToProps = dispatch => {
  return bindActionCreators({ getTravelerList, getMyPreviousList, myPreviouspostStartLoading, myPulltorefreshPreviousPost }, dispatch);

};
class MyPreviousPost extends Component{
  constructor(props) {
    super(props);
    this.state={
      data: [
    ...
    try {
      const value = AsyncStorage.getItem('user_id',(err, result) => {
        if (result !== undefined && result !== null && result !== '') {
          this.props.myPreviouspostStartLoading();
          this.props.getMyPreviousList(result);
        }
        });
    } catch (error) {
  ...
  _onRefresh() {
    ...
    this.props.myPulltorefreshPreviousPost();
    try {
      const value = AsyncStorage.getItem('user_id',(err, result) => {
        if (result !== undefined && result !== null && result !== '') {
          this.props.myPreviouspostStartLoading();
  ...
  _keyExtractor = (item, index) => item._id;
   render() {
     ...
     if (this.props.previousListResponse.data !== undefined && (this.props.previousListResponse.data === null || this.props.previousListResponse.data.length ===0)) {
     return(
    ...
    return(
      <View style={{flex:1,}}>
        <FlatList
          ...
          data={this.props.previousListResponse.data}
          renderItem={({item}) =>
...
export default connect(mapStateToProps,mapDispatchToProps )(MyPreviousPost);
